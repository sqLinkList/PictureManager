...
//    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    ...
    public BaseResponse<PictureVO> uploadPictureByUrl(
            ...
            HttpServletRequest request) {
        log.info("收到 URL 上传请求，fileUrl: {}", pictureUploadRequest != null ? pictureUploadRequest.getFileUrl() : "null");
        log.info("开始获取登录用户...");
        User loginUser = userService.getLoginUser(request);
        log.info("登录用户获取成功，userId: {}", loginUser.getId());
        String fileUrl = pictureUploadRequest.getFileUrl();
        log.info("准备调用 uploadPicture，fileUrl: {}", fileUrl);
        log.info("pictureService 类型: {}", pictureService.getClass().getName());
        try {
            PictureVO pictureVO = pictureService.uploadPicture(fileUrl, pictureUploadRequest, loginUser);
            log.info("uploadPicture 调用完成");
            return ResultUtils.success(pictureVO);
        } catch (Exception e) {
            log.error("uploadPicture 调用异常", e);
            throw e;
        }
    }
