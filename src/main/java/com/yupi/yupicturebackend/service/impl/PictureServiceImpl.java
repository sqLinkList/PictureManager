...
//    }
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        log.info("进入 uploadPicture 方法，inputSource 类型: {}", inputSource != null ? inputSource.getClass().getSimpleName() : "null");
        ...
        // 根据inputSource 的类型区分上传方式
        PictureUploadTemplate pictureUploadTemplate = filePictureUpload;
        if (inputSource instanceof String) {
            pictureUploadTemplate = urlPictureUpload;
        }
        UploadPictureResult uploadPictureResult = pictureUploadTemplate.uploadPicture(inputSource, uploadPathPrefix);
        log.info("图片上传成功，URL: {}", uploadPictureResult.getUrl());
        //构造入库图片信息
        ...
        // 调用混元大模型自动生成简介、标签、分类（异步超时 15 秒，超时则跳过）
        log.info("开始调用混元大模型分析图片...");
        try {
            CompletableFuture<ImageAnalysisResult> future = CompletableFuture.supplyAsync(
                    () -> hunyuanImageAnalysis.analyzeImage(uploadPictureResult.getUrl())
            );
            ImageAnalysisResult analysisResult = future.get(15, TimeUnit.SECONDS);
            log.info("混元大模型分析完成");
    ...
    public QueryWrapper<Picture> getQueryWrapper(PictureQueryRequest pictureQueryRequest) {
        ...
        if (StrUtil.isNotBlank(searchText)) {
            // 需要拼接查询条件
            // sql语句查询 name like "%xxx%" or introduction like "%xxx%"
